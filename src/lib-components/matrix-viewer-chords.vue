<template>
  <div class="matrix-viewer-chords">
    <div>
      <svg
        :class="quantumMatrixClass"
        :width="columnSize + (1.5 + 2) * size"
        :height="rowSize + (3.5 + coordNamesIn.length) * size"
      >
        <g :transform="`translate(${(1 + 2) * size}, ${1 * size})`">
          <matrix-labels
            :size="size"
            axis-label="input"
            location="top"
        ...
        <g :transform="`translate(${2 * size}, ${size})`">
          <matrix-dimensions
        ...
        <g :transform="`translate(${(1 + 2) * size}, ${(coordNamesIn.length + 4) * size})`">
          <matrix-labels
            :size="size"
            axis-label=""
            location="top"
        ...
        <g :transform="`translate(${(1 + 2) * size}, ${(coordNamesIn.length + 1) * size})`">
          <line
            ...
            :y1="2.5"
            :y2="- 2.5 + 3 * size"
            :style="{ stroke: generateColor(d.re, d.im), strokeWidth: rScale(d.re, d.im) / 4 }"
